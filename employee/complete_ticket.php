...
try {
    ...
    if ($result->num_rows === 0) {
        throw new Exception("无法完成工单：工单不存在或不属于您，或状态不正确");
    }

    // 更新工单状态
    $status = 'completed';
    $completed_at = date('Y-m-d H:i:s');
    $stmt = $conn->prepare("UPDATE tickets SET status = ?, completed_at = ? WHERE id = ?");
    if (!$stmt) {
        throw new Exception("准备更新失败: " . $conn->error);
    }

    $stmt->bind_param("ssi", $status, $completed_at, $ticket_id);
    if (!$stmt->execute()) {
    ...
    // 记录工单状态变更
    $description = "工单已被标记为完成";
    $stmt = $conn->prepare("INSERT INTO ticket_history (ticket_id, user_id, action, description) VALUES (?, ?, 'completed', ?)");
    if (!$stmt) {
    ...
    header('Content-Type: application/json');
    echo json_encode(['status' => 'success', 'message' => '工单已成功完成']);
